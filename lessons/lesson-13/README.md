...
- Install `spacy` with `conda` (or use `pip`)
    ```python
    conda install spacy
    ``` 
...
**Check:** What are some important features of decision trees and random forests?
  ...
  - Random forests are strong models that are made up a collection of decision trees
...
Scikit-learn has many pre-processing utilities that simplify many of the tasks required to convert text into features for a model. These can be found in the `sklearn.preprocessing.text` package.

We will use the StumbleUpon web crawl dataset again and perform a text classification task. Instead of using other features of the webpages, we will use the text content itself to predict whether or not the webpage is 'evergreen'.
...
Build a random forest model to predict the "evergreen-ness" of a website using the title features
...
**Check:** What does TF-IDF stand for?
- What does this function do and why is it useful?

> **A:** Term Frequency-Inverse Document Frequency 
- statistic that reflects how important a word is to a document in a collection or corpus

**Activity:** Use `TfidfVectorizer` to create a feature representation of the stumbleupon titles
